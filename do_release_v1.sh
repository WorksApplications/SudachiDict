...
# How to run: do_release_v1.sh DICT_VERSION /path/to/directory/with/raw/dictionaries aws_profile aws_mfa_uid
...
python3 "$SCRIPT_DIR/scripts/01_upload_raw_dictionaries.py" \
  ...
  --aws_mfa="$AWS_MFA_ID" \
  --dictionary_format v1

# build binary dictionaries
"$SCRIPT_DIR/gradlew" -Pdict.version="$DICT_VERSION" -Pdict.format=v1 build
